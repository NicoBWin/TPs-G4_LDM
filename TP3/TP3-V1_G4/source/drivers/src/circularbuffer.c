...
void buff2DAC_callback ()
{
	...
	//bitstream_modulate(1, false);
	DAC_SetData(DAC_0,FSK_get_sample(modulating_bit));
}
...
void ADC2BIT_callback ()
{
	...
	{
		//bit[i] = ADC_getData (ADC_0);
		bit_pre_filter[i] = ADC_getData (ADC_0);
		bit[i] = delay_filter( bit_pre_filter[i]  ) ; // obtengo la nueva data cada 1/fs
	}
	if (i == 13)
	{
		i = 0;
		compoutput2UART_callback(bit);
	}
	else
	{
		i++;
