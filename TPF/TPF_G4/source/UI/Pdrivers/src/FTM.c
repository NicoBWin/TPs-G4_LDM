...
#define PORT_Alt4	0b100
#define PORT_Alt6	0b110

//#define FTM0_CH0	PORTNUM2PIN(PA,3)	//ALT3
#define FTM0_CH0	PORTNUM2PIN(PC,1)	//ALT4
...
void FTM_Init (FTM_n FTMn, FTMConfig_t config) {
    ...
    if(config.channel == FTM_Channel_0 & FTMn == FTM_0){
    	PORTC->PCR[PIN2NUM(FTM0_PIN[FTM_Channel_0])] = 0x0; //Clear all bits
    	PORTC->PCR[PIN2NUM(FTM0_PIN[FTM_Channel_0])] |= PORT_PCR_DSE(1);
    	PORTC->PCR[PIN2NUM(FTM0_PIN[FTM_Channel_0])] |= PORT_PCR_MUX(PORT_Alt4); //Set MUX to Alt4
    }
    else {
	...
	if(config.mode == FTM_mInputCapture) {
		// Enable Overflow interrupt
		FTMp->SC |= FTM_SC_TOIE_MASK;
	}
...
ISR_t FTM0_IRQHandler(void){
	//IC_ISR(FTM_0);
}

ISR_t FTM1_IRQHandler(void){
	//IC_ISR(FTM_1);
}

ISR_t FTM2_IRQHandler(void) {
	IC_ISR(FTM_2);	//For InputCapure purpose
}

ISR_t FTM3_IRQHandler(void){
	//IC_ISR(FTM_3);
}
